...
| Method | 端点 | Description |
|--------|----------|-------------|
| POST | `/api/scan-url` | 扫描 a single URL |
| POST | `/api/scan-request` | 扫描 a raw HTTP request |
| POST | `/api/scans/run` | Trigger a target-based background scan |
| GET | `/api/scan/status` | Current scan status |
| POST | `/api/scan-records` | 扫描 specific HTTP records by UUID |
| POST | `/api/scan-all-records` | 扫描 all in-scope HTTP records |
...
| GET | `/api/projects/:uuid/stats` | 项目 statistics |

### [OAST Interactions](api-references/oast-interactions.md)

| Method | 端点 | Description |
|--------|----------|-------------|
| GET | `/api/oast-interactions` | List OAST（带外交互） interactions |
| GET | `/api/oast-interactions/:id` | Get OAST（带外交互） interaction detail |
| DELETE | `/api/oast-interactions/:id` | Delete OAST（带外交互） interaction |
...
Cloud-storage endpoints for source bundles and result archives. Require `storage` to be enabled in `vigolium-configs.yaml`.
...
| GET | `/api/agent/sessions/:id/logs` | Tail or stream `runtime.log` (SSE（服务器推送事件）-capable) |
