...
4. **Verify session directory permissions.** Agent sessions are stored under `~/.vigolium/agent-sessions/` 默认情况下 (configurable 通过 `agent.sessions_dir` in `vigolium-configs.yaml`). Ensure this directory is writable.
