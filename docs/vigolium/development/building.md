...
> binary in the working tree. Use `make build` (→ `bin/vigolium`) or
...
| [Bun](https://bun.sh) | Compiles the JS analysis engine embedded 通过 `//go:embed` (`make ensure-jsscan`) |
...
另见: [project-structure.md](project-structure.md) and
