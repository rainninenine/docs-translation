...
```
cmd/vigolium/        Main CLI entry point (Cobra)
...
platform/            External tooling — do NOT read/modify (except platform/vigolium-workbench, the Next.js UI)
...
| Path | Responsibility |
|------|----------------|
| `cmd/vigolium/` | `main()` — thin; delegates to `pkg/cli` |
...
| `internal/config/` | 设置 model, loader, scope matcher, agent config |
...
| `pkg/modules/` | 模块 interfaces, the **Registry**, and all active/passive scanner modules. `modkit/` = shared base types; `infra/` = block detection & request filtering; `modtest/` = unit-test helper |
...
| `pkg/dedup/` | Disk-backed dedup sets and 每-scan dedup manager |
...
| `pkg/oast/` | Out-of-band (OAST（带外交互）) interaction service for blind-vuln detection |
...
| `pkg/harvester/` | 端点/secret harvesting helpers |
...
| `pkg/audit/` | Embedded vigolium-audit harness driver (separate from olium) |
...
| Path | Responsibility |
|------|----------------|
| `pkg/database/` | Repository pattern over SQLite (default) / PostgreSQL 通过 Bun ORM; batch record ingestion & 每-source dedup |
...
All scan data is scoped to a **project** 通过 `project_uuid` on every data table.
...
另见: [building.md](building.md) and
