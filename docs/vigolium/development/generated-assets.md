...
这些是 deterministic and must stay in sync with their source:
...
这些是 non-deterministic binary/bundle outputs. They are regenerated from
...
|---|---|---|
| jsscan binaries (embedded 每-platform) | `internal/resources/deparos/jsscan/`, `public/presets/deparos/jsscan/` | `make ensure-jsscan` / `make update-jsscan` |
...
embedded 通过 `//go:embed`), which is why CI runs it before any `go build`.

## Linting

Generated files are excluded from `golangci-lint` 通过 the `exclusions.generated:
