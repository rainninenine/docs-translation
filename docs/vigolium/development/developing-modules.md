...
| 扫描范围 | When it fires |
|-------|---------------|
| `ScanScopeInsertionPoint` | once 每 parameter/header/cookie/body insertion point |
| `ScanScopeRequest` | once 每 unique request |
| `ScanScopeHost` | once 每 unique host |
...
(logic). 以下是 the shape of the real `cache-deception` module
...
- **Use `scanCtx` helpers**: `GetOrFetchBaseline`, `WildcardProbe`,
  `DedupMgr()`, and the OAST（带外交互） provider for blind detection.
...
| `TimeoutHinter` | `TimeoutHint() time.Duration` — raise the 每-module timeout for legitimately slow analysis (e.g. timing-based DiffScan) |
...
另见: [project-structure.md](project-structure.md) for where things live and
