...
All scan endpoints support multi-tenancy 通过 the `X-Project-UUID` header. When provided, scans are created and queried within 指定的 project. Responses include the `project_uuid` field to confirm the project context.
...
启动 an asynchronous scan of a single URL. Equivalent to the CLI `scan-url` command. 返回 `202 Accepted` immediately with a scan ID.
...
启动 an asynchronous scan from a base64-encoded raw HTTP request, optionally paired with a base64-encoded raw HTTP response (Burp-style). Equivalent to the CLI `scan-request` command. 返回 `202 Accepted` immediately with a scan ID.
...
| `http_response_base64` | string | No       | Base64-encoded raw HTTP response — when provided, it's attached to the request for passive analysis 而不是 re-fetching |
...
Triggers a background scan against target URLs. Equivalent to `vigolium scan -t <url>`. At least one target URL is required — use `POST /api/scan-all-records` to scan existing DB records.

返回 `202 Accepted` on success, `200 OK` for dry runs, or `409 Conflict` if a scan is already running.
...
| `modules`              | string[]          | No       | 模块 IDs with fuzzy match (like `-m`)                                     |
...
| `max_per_host`         | int               | No       | Maximum concurrent requests 每 host                                        |
| `rate_limit`           | int               | No       | Maximum request submissions 每 second                                      |
...
| `scope_origin`         | string            | No       | 扫描范围 origin mode: `all`, `relaxed`, `balanced`, `strict`                   |
...
| `intensity`            | string            | No       | 扫描 intensity preset: `"quick"`, `"balanced"`, or `"deep"`. 解析 to `scanning_profile` + `strategy`. Explicit `scanning_profile` or `strategy` fields override intensity. |
...
When the scan queue is enabled and another scan is already running, new scans are queued 而不是 returning `409`:
...
When a scan starts (通过 API or CLI), the runner logs a configuration summary to stderr showing the effective settings. 这很有用 for verifying that `scanning_max_duration` and 每-phase duration factors are applied correctly.
...
返回 the status of 当前 or most recent scan. The `status` field reflects pause state 适用时.
...
| `selective`   | `POST /api/scan/records`                 | 扫描 a specific set of HTTP records by UUID                                                    |
...
返回 paginated scan history ordered by creation date (newest first).
...
停止 a specific running scan by UUID. The scan must be the currently active scan. Workers finish their current tasks before fully stopping.
...
|------|--------------------------------------------|
| 400  | Missing UUID                               |
| 409  | No scan running, or UUID is not 当前 scan |
...
|------|--------------------------------------------------|
| 400  | Missing UUID                                     |
| 409  | No scan running, UUID is not 当前 scan, or scan is already paused |
...
|------|--------------------------------------------------|
| 400  | Missing UUID                                     |
| 409  | No scan running, UUID is not 当前 scan, or scan is not paused |

---

### GET /api/scans/:uuid/logs — Get Scan Logs

返回 log output for a scan. The endpoint serves one of two response formats 取决于 what the server has on disk:

1. **`text/plain` runtime.log (preferred)** — When the scan's session directory contains a `runtime.log` file (默认 for scans launched 通过 the server today, including `/api/scan-url`, `/api/scan-request`, and `/api/scans/run`), the raw log is streamed as `text/plain; charset=utf-8`. ANSI color codes are preserved 默认情况下; pass `?strip=1` to strip them. This matches what the CLI `vigolium log` command streams.
...
- **Raw console output** (`trace`): Every line printed to the terminal during the scan, with ANSI color codes stripped. 这包括 phase headers, traffic lines (`❯ spider │ [200] GET ...`), progress feedback, and scan configuration banners.
...
| `phase`      | string | 扫描 phase (see table below), or empty for global events (scan start/finish, pause/resume)        |
...
| Phase                | Description                                                       |
|----------------------|-------------------------------------------------------------------|
| `config`             | 扫描 configuration snapshot (logged once at scan start)           |
...
启动 an asynchronous scan on specific HTTP records identified by UUID. 返回 `202 Accepted` on success or `409 Conflict` if a scan is already running. Only one scan can run at a time.
...
| `headers`              | map[string]string | No       | Custom HTTP headers included in all requests                                |
| `scanning_profile`     | string            | No       | Scanning profile name or path                                               |
| `intensity`            | string            | No       | 扫描 intensity preset: `"quick"`, `"balanced"`, or `"deep"`. 解析 to `scanning_profile` + `strategy`. |
