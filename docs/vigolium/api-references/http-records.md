# Vigolium API Reference — HTTP Records

## GET /api/http-records — List HTTP Records

返回 paginated HTTP request/response records stored in the database. Response and request bodies are excluded from list responses 出于性能考虑.
...
删除 a single HTTP record by UUID. Associated `finding_records` junction rows are also removed.
