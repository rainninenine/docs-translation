...
返回 paginated vulnerability findings.
...
> **Note:** The list response excludes the heavy fields `request`, `response`, and `additional_evidence` 出于性能考虑. Use `GET /api/findings/:id` to fetch a finding with its full evidence payload.

---

## GET /api/findings/:id — Get Finding Detail

返回 a single finding by its numeric ID, including the full `request`, `response`, and `additional_evidence` payloads.
...
删除 a single finding by its numeric ID. Associated `finding_records` junction rows are also removed.
...
Lifecycle state of the finding. One of: `draft`, `triaged`, `false_positive`, `accepted_risk`, `fixed`. Update 通过 `PATCH /api/findings/:id/status` and filter on the list endpoint with `?status=`.
...
The repository name or URL associated with a finding. Primarily populated for audit findings. During audit import, the value is resolved with 以下 priority:

1. `repo_url` field in `audit-state.json` (如果存在)
2. `repo` field in `audit-state.json` (如果存在)
