# Vigolium API Reference — Ingestion

## POST /api/ingest-http — Ingest HTTP Data

Import HTTP request/response data into the database for scanning. 支持 multiple input formats.
...
| `X-Project-UUID`  | No       | 项目 UUID to scope the imported data. Falls back to 默认 project if omitted. |
...
Use the `X-Project-UUID` header to scope imported data to a project. If omitted, data is stored under 默认 project.
