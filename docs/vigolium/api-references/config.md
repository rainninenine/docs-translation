# Vigolium API Reference — Config

## GET /api/config — View Configuration

返回 the full server configuration as flattened dot-notation key-value pairs. Sensitive values (API keys, passwords, tokens) are redacted 默认情况下.
...
更新 一个或多个 configuration values using dot-notation keys. Values are coerced to match the existing field type (bool, int, float, string, or comma-separated list). Changes are persisted to the config file on disk.
...
The server watches the config file (`~/.vigolium/vigolium-configs.yaml`) for changes. When the file is modified — 是否 by a text editor, the CLI (`vigolium config set`), or any other tool — reloadable sections are automatically applied without restarting the server.
...
Changes made 通过 the API (`POST /api/config`, `POST /api/scope`) do not trigger a redundant reload.
