...
The storage API provides cloud object storage integration for source code upload/download and scan result archival. All objects are scoped to a project 通过 the `X-Project-UUID` header and stored under `<bucket>/<project-uuid>/`.

存储 is **disabled 默认情况下**. Enable it by setting `storage.enabled: true` in `vigolium-configs.yaml`. All endpoints return `503 Service Unavailable` when storage is not configured.

| 端点                              | Method | Role     | Description                              |
|---------------------------------------|--------|----------|------------------------------------------|
...
The `storage_url` field on the 扫描 / AgenticScan DB record is updated with the `gs://` URL after upload.

---

## POST /api/storage/upload-source

上传 a source code archive to cloud storage, scoped to the project.
...
下载 a previously uploaded source file.
...
返回 the file as `application/octet-stream` with `Content-Disposition: attachment`.

返回 `400` if the key contains path traversal sequences.

---

## GET /api/storage/results/:scan-uuid

下载 the result bundle for a native scan or agentic scan. Searches `native-scans/<uuid>/results.tar.gz` first, then `agentic-scans/<uuid>/results.tar.gz`.
...
返回 `application/gzip` with `Content-Disposition: attachment`.

返回 `404` if no results have been uploaded for the given UUID.
...
生成 a presigned URL for direct upload or download, bypassing the API server. Useful for large files or client-side uploads.
...
When `upload_results` is enabled, the `storage_url` field is populated on the 扫描 or AgenticScan record after upload completes.
