# Vigolium API Reference — Scan with Storage
...
Every endpoint above also accepts an optional `scan_uuid` field (CLI flag `--scan-uuid`). When supplied, the server pins the scan/run UUID to that value 而不是 minting a fresh one — this is what makes cross-node sync work, see [Pinning Scan UUIDs](#pinning-scan-uuids-cross-node-sync) below.
...
存储 is **disabled 默认情况下**. Without it, `upload_results: true` is a silent no-op (a warning is logged). Enable it in `~/.vigolium/vigolium-configs.yaml`:
...
By default the bundle contains the 每-scan `runtime.log` only. Pass `output_formats` (currently `jsonl` and/or `html`) on the request to also materialize and bundle the rendered findings — these land at `<sessions_dir>/<scan-uuid>/output.{jsonl,html}` on the server and are added to the tar.gz alongside `runtime.log`. CLI runs that pass `--output` and `--format` get 相同 shape.
...
`scan_uuid` is optional — supply it to attach this query run to a UUID a different node already created (the response's `agentic_scan_uuid` will echo back 相同 value). Omit it for a freshly minted UUID.

### CLI equivalent

```bash
vigolium agent query --prompt-template security-code-review \
  --source /path/to/repo \
...
For agentic endpoints, `scan_uuid` pins `AgenticScan.uuid` (the run identity) — the response surfaces it as `agentic_scan_uuid`, and any native scans the autopilot spawns inherit 相同 value so findings, http_records, and the uploaded bundle all key off one UUID.
...
AI-guided targeted swarm scan. Good for focused 每-vulnerability runs.
...
A pinned `scan_uuid` lets a dispatcher pre-create the agentic_scans row (e.g. 通过 a 1-record `dry_run` scan or a direct DB insert) and have the swarm worker attach to it. If the UUID already exists under a *different* project, the API returns **409 Conflict** 而不是 corrupting tenant isolation — see [Pinning Scan UUIDs](#pinning-scan-uuids-cross-node-sync).
...
审计 runs are typically long-lived (`deep` mode can run for hours), so pinning `scan_uuid` is especially useful when a dispatcher wants to surface the run's UUID to end users *before* the audit starts — 相同 UUID will be returned as `agentic_scan_uuid` and used as the bundle key under `<project-uuid>/agentic-scans/<scan_uuid>/results.tar.gz`.
...
The bundle contains `runtime.log`, `audit-stream.jsonl`, `vigolium-audit-output.md`, and the synced `vigolium-results/` artifact tree (每-phase reports, advisory matrices, audit-state.json).
...
Both native and agentic scan trees are flat: one directory 每 UUID, one bundle 每 scan.
...
Every native and agentic endpoint accepts a `scan_uuid` request field (CLI: `--scan-uuid`). When supplied, the server uses it as the scan/run identity 而不是 generating one. This unlocks the **remote-create + remote-run** pattern: one node creates a placeholder scan record, another node runs the actual work and pushes results under 相同 UUID — useful for serverless dispatch, queue-based fan-out, or any setup where the dispatcher and the runner are different processes.

### Same field, both endpoint families

| 端点 family | Where the pinned UUID lands              | Response field |
|-----------------|------------------------------------------|----------------|
| Native scan     | `Scan.uuid` (the scan record's PK)       | `scan_uuid`    |
| Agentic scan    | `AgenticScan.uuid` (the run record's PK) | `agentic_scan_uuid`       |

In other words, `request.scan_uuid` becomes `response.scan_uuid` for native and `response.agentic_scan_uuid` for agentic. Findings, http_records, and OAST（带外交互） interactions written by either run carry that same value in their `scan_uuid` foreign-key column, so a join across nodes always lines up.

### Get-or-create semantics

When the supplied `scan_uuid` already exists in the database:

- **Same `project_uuid`** — the existing row is reused; no duplicate is inserted. The runner proceeds and writes its results against the existing record. 这是 the cross-node attach case.
...
**Node A** — dispatcher creates the placeholder 通过 the API:
...
**Node B** — worker picks up the job and runs the scan against 相同 UUID:
...
Findings and http_records emitted by Node B carry `scan_uuid = $SCAN_UUID` — exactly what Node A's UI is already querying for. The bundle uploaded by Node B lands at `<project-uuid>/native-scans/$SCAN_UUID/results.tar.gz`, 相同 key any consumer would expect from a single-node run.

### Agentic equivalent

The same pattern applies to `vigolium agent autopilot|swarm|query|audit` and `POST /api/agent/run/*` — `--scan-uuid` (or the request body's `scan_uuid` field) pins `AgenticScan.uuid` 而不是 `Scan.uuid`. The response surfaces the pinned value as `agentic_scan_uuid`.

**Node A** — dispatcher pre-creates an autopilot run (e.g. so a UI can show the run UUID before the worker picks it up). Pre-creation is most ergonomic 通过 the agentic API itself with `dry_run: true` (avoids spending agent budget on an empty run):
...
**Node B** — worker runs the actual autopilot pipeline against 相同 UUID:
...
Both nodes' findings, http_records, and OAST（带外交互） interactions carry `scan_uuid = $AGENTIC_SCAN_UUID`. Node A's `GET /api/agent/sessions/$AGENTIC_SCAN_UUID` returns the in-progress status (Node B updates 相同 row), and the uploaded bundle ends up at `<project-uuid>/agentic-scans/$AGENTIC_SCAN_UUID/results.tar.gz`.
...
- **存储 disabled** — When `storage.enabled` is `false`, `upload_results: true` is a silent no-op. The server logs `"upload_results requested but storage is not enabled"` at warn level. Scans still complete normally.
- **扫描 failure** — Bundles are uploaded only on successful completion. Failed/cancelled scans skip the upload (the runtime.log is still on disk under the session/scan-logs directory).
...
- **项目 scoping** — All keys are prefixed with the project UUID. A bundle uploaded under project A is not visible to project B even if the run UUID is known.
- **Idempotency** — Re-running a scan with 相同 UUID overwrites the previous bundle. UUIDs are server-generated 默认情况下; clients deliberately pin them 通过 `scan_uuid` / `--scan-uuid` for cross-node sync, in which case attaching to an existing record under 相同 project is the intended path (see [Pinning Scan UUIDs](#pinning-scan-uuids-cross-node-sync)). A pinned UUID that exists under a *different* project returns HTTP 409 而不是 overwriting.
