...
Manage projects for multi-tenant data isolation. All scan data (HTTP records, findings, scopes, scans) is scoped to a project 通过 `project_uuid`. Projects can optionally carry `allowed_emails` (exact addresses) and `allowed_domains` (email domain patterns) to control access.
...
When a request includes both `X-Project-UUID` and `X-User-Email` headers, the server checks 是否 the user is allowed to access the project:
...
返回 all projects with aggregated statistics. Optionally filter by owner UUID.
...
| `stats.oast_interactions` | int   | Total OAST（带外交互） (out-of-band) interactions    |

**Errors:**

| Code | Condition              |
|------|------------------------|
| 503  | 数据库 not connected |
...
|-------------------|----------|----------|--------------------------------------------------|
| `name`            | string   | Yes      | 项目 name                                     |
| `description`     | string   | No       | 项目 description                              |
...
返回 a mapping of email domains and exact email addresses to project UUIDs. Useful for frontend middleware to resolve which projects a user has access to in a single call.
...
Delete a project by UUID. The default project (`00000000-0000-0000-0000-000000000001`) cannot be deleted. All data (scans, HTTP records, findings, scopes, source repos, OAST（带外交互） interactions, scan logs) belonging to the deleted project is automatically reassigned to 默认 project.
