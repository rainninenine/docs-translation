# Vigolium API Reference — Generic Database API

Unified CRUD API for interacting with any database table. 返回 raw JSON and supports pagination, filtering, sorting, column selection, and full-text search across all tables.

Read endpoints (GET) require **viewer** role or above. Write endpoints (POST, PUT, DELETE) require **admin** role.

项目-scoped tables (`scans`, `http_records`, `findings`, `source_repos`, `session_hostnames`, `oast_interactions`, `scan_logs`, `agentic_scans`, `scopes`) automatically filter by the `X-Project-UUID` header. Pass `?all_projects=true` to disable project scoping (admin use).

---

## GET /api/db/tables — List All Tables

返回 all database tables with their row counts.
...
返回 column metadata and primary key information for a specific table.
...
返回 paginated, filtered, sorted records from any table.
...
返回 a single record by its primary key value. Only works for tables with a single-column primary key.
...
Inserts a new record into 指定的 table. 需要 **admin** role.
...
| 500  | 数据库 constraint violation                |

---

## PUT /api/db/tables/:table/records/:id — Update Record

更新 一个或多个 fields on an existing record. 需要 **admin** role.
...
删除 a single record by primary key. 需要 **admin** role.
