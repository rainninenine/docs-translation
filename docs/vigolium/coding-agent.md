...
- Two JSON contracts:
  - **`-j/--json`** on read/query commands (`finding`, `traffic`, `db`) → a single
    structured object with **compact, token-aware** bodies. 这是 what you parse.
  - **`--format jsonl`** / `export` → the bulk `{"type":...,"data":{...}}` stream
    (one object 每 line), full fidelity. Use for archival/bulk, not triage.
- Non-interactive 默认情况下: the TUI is opt-in (`--tui`), never auto-launched.
...
- Binary/static bodies (images, fonts, JS bundles, gzip) are stubbed as
  `{"body_omitted":"binary", ...}`.
- Findings get a `response_evidence` snippet windowed around the match 而不是
  the whole page.
...
过滤 (shared with `traffic` / `db ls`): `--host --path --method --status
...
`finding` and `traffic` can read a file directly 而不是 your project DB —
...
every row in the file is shown 无论 the `project_uuid` it carries.
...
`sqlite3`, `db`) — it dumps the 每-run DB to `<output>.sqlite` and combines with
other formats. Under `--split-by-host` each 每-host file is named
`<base>-<host>.sqlite`:
...
(the `--db` target, or 配置的 default when `--db` is omitted). The source
...
scans, agentic scans, and OAST（带外交互） interactions all flow in, deduped on their natural
keys (records by UUID, findings by `(project_uuid, finding_hash)`), so re-running
相同 import adds nothing. Each row keeps its original `project_uuid`.
...
这是 the natural companion to `vigolium scan -S --format sqlite` above: fan out
scans into standalone 每-host `.sqlite` files, then merge them back into one
...
要监视 traffic land as files while another tool (Burp, a proxy, `vigolium ingest`)
...
except the indexes are append-only **`index.jsonl`** (one object 每 line — tail/grep
it live) and 每-host ids resume across server restarts. Point your agent at `./mirror`
and let it `jq`/`grep` the growing tree.

## Gotchas

- `-S` means `--stateless` on `scan` but `--scan-on-receive` on `ingest`.
- `--json` (compact, single object) ≠ `--format jsonl` (bulk, one line 每 row).
- With `-P/--parallel`, `--fail-on` is evaluated 每 child process.
