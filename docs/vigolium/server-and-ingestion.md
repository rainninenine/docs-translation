...
This guide covers how to start Vigolium in server mode and ingest HTTP traffic into the database using the REST API and CLI.
...
The server listens on `0.0.0.0:9002` 默认情况下.
...
| Value | Behavior |
|-------|----------|
| `reflect-origin` (default) | Echoes the requesting `Origin` header back. 允许 credentials. |
| `*` | 允许 all origins without credentials (standard wildcard). |
| _(empty string)_ | 禁用 CORS middleware entirely. |
| `https://app.example.com, https://admin.example.com` | Comma-separated allowlist. 允许 credentials. |

## Project Scoping

All server operations are scoped to a project 通过 the `X-Project-UUID` request header. If omitted, 默认 project is used.
...
| GET | `/api/http-records` | 查询 stored HTTP records |
| GET | `/api/findings` | 查询 scan findings |
...
After ingesting HTTP records, trigger a vulnerability scan 通过 the API.
...
返回 `202 Accepted` on success, `409 Conflict` if a scan is already running.
...
At least one of `prompt_template`, `prompt_file`, or `prompt` is required. 返回 `202 Accepted` on success. Set `"stream": true` for real-time SSE（服务器推送事件） output.
...
SSE（服务器推送事件） events are `data:` lines with JSON payloads: `{"type":"chunk","text":"..."}` for real-time output, `{"type":"phase","phase":"..."}` for swarm phase transitions, `{"type":"done","result":{...}}` on completion, or `{"type":"error","error":"..."}` on failure.
