...
| Canary tests | `test/e2e/` | Original 每-app vulnerability detection tests |
| **基准测试 tests** | `test/benchmark/` | Data-driven module coverage validation (whitebox, blackbox, xbow) |
| Integration tests | `test/benchmark/xss_scanner/` | Brutelogic XSS gym (external) |

The benchmark system is designed to eventually supersede the 每-app canary tests in `test/e2e/` (dvwa_test.go, vampi_test.go, juiceshop_test.go) by providing 相同 coverage through YAML definitions with less boilerplate.

---

## Target Applications

Vigolium benchmarks against a diverse set of intentionally vulnerable applications. Each application covers different vulnerability categories and tech stacks.
...
| **NextJS VulnExamples** | Next.js / PostgreSQL | Missing 身份验证, Missing Authorization, Secrets Exposure, Stored XSS | Built from source | 3000 |
...
This application is an educational project demonstrating six categories of security flaws in a Next.js + PostgreSQL stack. It provides both **vulnerable implementations** and **secure solutions** 每个 category, making it valuable 两者 positive detection and negative (false positive) testing.

| Example | Vulnerability | Vulnerable Route | Type | What's Wrong |
|---------|--------------|-----------------|------|-------------|
| 1 | Missing 身份验证 | `GET /api/example-1-.../vulnerable` | Route Handler | No session token check — returns blog posts to anyone |
| 2 | Missing 身份验证 | `GET /example-2-.../vulnerable` | Server Component | No session check — queries DB and renders directly |
| 3 | Missing Authorization | `GET /api/example-3-.../vulnerable` | Route Handler | 检查 auth but returns ALL users' unpublished posts |
...
| **IBM Testfire** | `demo.testfire.net` | SQLi, XSS, 身份验证 Bypass |
...
2. **模块 resolution by ID**: Test cases reference modules by their registry ID (e.g., `active-sqli-error-based`). The harness resolves them from `modules.DefaultRegistry`.
3. **扫描 type dispatch**: The harness checks `module.ScanScopes()` to dispatch to the correct method:
   - `ScanScopeInsertionPoint`: 创建 insertion points 通过 `httpmsg.CreateAllInsertionPoints()`, filters by `AllowedInsertionPointTypes()`, calls `ScanPerInsertionPoint()` 每个.
...
5. **App-specific auth**: Some apps (e.g., DVWA) require authentication before vulnerability pages work. The `SetupAppAuth()` function dispatches 每-app setup (DB init, CSRF token extraction, login) and returns headers (cookies) to inject into all test cases.
6. **Network init safety**: `network.Init()` is called once 每 process 通过 `sync.Once` to avoid LevelDB close/reopen issues when running multiple test functions sequentially.
...
- **`XBOW_SOURCE_DIR` environment variable**: Must point to the root of the validation-benchmarks checkout (e.g., `/path/to/validation-benchmarks`). Set 通过 environment or passed through the Makefile.
...
- **No Docker needed**: 测试 run against public websites
...
| `xbow` | Docker Compose CLI builds from source, starts/stops automatically | Auto-discovered 通过 `docker compose port` |
...
- **模块 IDs** must match exactly what's registered in `pkg/modules/default_registry.go`. Run `go test -v -run TestResolveActiveModules ./test/benchmark/harness/...` to verify module resolution.
...
| `CoverageReport` | 模块 coverage matrix |
...
| `SetupTestInfraWithOAST()` | Initialize with OAST（带外交互） mock provider |
| `StartContainer(ctx, config)` | Start a Docker container 通过 testcontainers-go |
...
5. **Test execution**: 运行 active test cases (GET or POST) through the standard harness
...
XBOW definitions use the `build_context`, `service_name`, and `internal_port` fields 而不是 `image` and `port`:
...
The `${XBOW_SOURCE_DIR}` environment variable is expanded 通过 `os.ExpandEnv()` when the YAML is loaded. The Makefile sets it automatically, or it can be set in your shell.

### Phase 1 Benchmarks (13 total)

| ID | XBOW | Vuln Type | 端点 | Service:Port | 模块 | Assertion |
|----|------|-----------|----------|-------------|--------|-----------|
...
测试 are skipped automatically if `XBOW_SOURCE_DIR` is not set or the directory is inaccessible.
...
- 模块 IDs are case-sensitive and use kebab-case (e.g., `active-sqli-error-based`)
...
- Wait 所有 services: `make crapi-status` (all should show "healthy")
...
- Or pass it 通过 make: `make test-xbow XBOW_SOURCE_DIR=/path/to/validation-benchmarks`
...
- Some benchmarks (XBEN-083-24, XBEN-071-24) have database services that need time to initialize. Increase `startup_timeout` 如果需要.
...
4. 检查 the module's `ScanScope` matches how the endpoint should be scanned:
   ...
   - `PerHost` modules (CORS) are called once 每 unique host
...
| 模块 | Reason | Workaround |
|--------|--------|------------|
| `active-http-request-smuggling` | 需要 specific server configurations (CL.TE, TE.CL) | Manual testing with custom servers |
