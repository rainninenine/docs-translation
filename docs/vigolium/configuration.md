...
The main config file is `~/.vigolium/vigolium-configs.yaml`. 它是 created automatically on first run with sensible defaults.

Vigolium searches for configuration in this order:

1. Path specified 通过 the `--config` flag (error if not found)
...
3. **Scanning profile** -- loaded 通过 `--scanning-profile <name>` (from `~/.vigolium/profiles/`)
4. **项目-level config** -- 每-project overlay at `~/.vigolium/projects/<uuid>/config.yaml`
...
On startup Vigolium compares the running version against the latest `@vigolium/vigolium` release on the npm registry (cached to at most once 每 24 h in `~/.vigolium/update-check.json`). When a newer version exists it prints a one-line notice at the end of the run:
...
控制 which scan phases run 每个 strategy preset.
...
404 fingerprinting, Wayback harvesting). Empty (默认) keeps the built-in
...
Centralized speed control. Common values serve as baselines; 每-phase subsections override them.
...
> **Total scan cap (CLI):** `max_duration` above is only the 每-phase base — on its
...
> time of the whole scan at the value you pass: the 每-phase `duration_factor`s then
> distribute that budget across phases (and remaining phases are skipped once it
> elapses) 而不是 each phase getting the full value sequentially. A single-phase
...
控制 which scanner modules run and JavaScript extension settings.
...
定义 what is in scope for scanning. Exclude rules take priority over include rules.
...
Override the provider 每 run with `--provider openai-codex-oauth | anthropic-api-key | openai-api-key | anthropic-cli`
(autopilot) or 通过 `agent.olium.provider` in config. Model overrides use `--model`
on `agent autopilot` and `agent olium`.

### `database`

存储 backend. SQLite is 默认; PostgreSQL is supported for multi-user deployments.
...
> ships only public base URLs / feature flags) 而不是 forking the upstream
...
控制 how parameter values are mutated during active scanning.
...
Out-of-Band Application Security Testing 通过 interactsh callbacks.
...
| webhook  | One POST 每 scan        | No                 | Programmatic integrations, CI/CD, dashboards |
...
| `telegram`       | Only 每-finding telegram messages fire |
| `discord`        | Only 每-finding discord messages fire |
...
state (`completed` or `failed`). One request 每 scan, JSON body, retried up
...
The webhook is best configured **每 project** so each project can post to
its own endpoint. Set it 通过 project config overlay:
...
**Headers:** `Content-Type: application/json` and (设置时)
...
This loads `~/.vigolium/profiles/aggressive.yaml` and overlays it onto 当前 config. Only non-zero fields in the profile override the base config; unspecified fields are left unchanged.

Built-in profiles are bundled in `public/presets/profiles/`. See [native-scan/scanning-modes-overview.md](native-scan/scanning-modes-overview.md) 详情.

## Project-Level Config

Each project can have its own config overlay at `~/.vigolium/projects/<uuid>/config.yaml`. This uses 相同 format as scanning profiles and is automatically applied when the project is active.
