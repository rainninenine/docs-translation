...
```bash
osmedeus cloud run -f fast -t example.com                          # 单目标
...
osmedeus cloud run -f fast -t example.com --reuse                  # 复用基础设施
...
osmedeus cloud setup --reuse-with "1.2.3.4"   # 设置现有机器
```
...
osmedeus cloud config set providers.aws.use_spot true          # 便宜 70%
```
